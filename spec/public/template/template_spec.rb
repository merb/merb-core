...
describe Merb::Template do
  ...
  it "should compile and inline templates that comes through via VirtualFile" do
    ...
    template_path = File.dirname(__FILE__) / "templates" / "virtualinline.html.erb"
    Merb::Template.inline_template(VirtualFile.new("Hello Inline", template_path), [], Merb::Test::Fixtures::MyHelpers)

    Merb::Test::Fixtures::Environment.new.send(Merb::Template.template_name(template_path)).should == "Hello Inline"
  end
