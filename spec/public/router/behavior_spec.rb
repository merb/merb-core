...
describe Merb::Router::Behavior do
  ...
  describe "#match" do
    it "should work with and without an explicit block parameter" do
      Merb::Router.prepare do
        match('/fruit') do
          match('/banana').to(:controller => 'fruit', :action => 'banana')
        end

        match('/pets') do |b|
          b.match('/dogs').to(:controller => 'pets', :action => 'dogs')
        end
      end

      route_for('/fruit/banana').should have_route(:controller => 'fruit', :action => 'banana')
      route_for('/pets/dogs').should have_route(:controller => 'pets', :action => 'dogs')
    end
  end
end
